...
//     }
// );
document.querySelector('.nav-buttons').addEventListener('click', async (event) => {
    ...
    if (data.recipes.length > 0) {
      ...
      data.recipes.forEach(recipe => {
        ...
        card.innerHTML = `
          ...
          <p><a href="${recipe.source_url}" target="_blank">View Full Recipe</a></p>
        `;
        container.appendChild(card);
      });
    } else {
      container.innerHTML = `No ${query} recipes found.`;
    }
  }
});
